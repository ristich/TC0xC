...
cursor.execute("SELECT attacker_id, SUM(damage_points) AS total_damage FROM hits GROUP BY attacker_id ORDER BY total_damage DESC LIMIT 25")
damage_dealt_leaderboard = cursor.fetchall()

# Retrieve the leaderboard data for damage taken
cursor.execute("SELECT user_id, SUM(damage_points) AS total_damage FROM hits GROUP BY user_id ORDER BY total_damage DESC LIMIT 25")
